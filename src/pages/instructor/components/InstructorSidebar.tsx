...
const InstructorSidebar: React.FC<InstructorSidebarProps> = ({
  ...
  return <>
      {/* Mobile Backdrop */}
      {isOpen && <div className="fixed inset-0 bg-foreground/60 backdrop-blur-sm z-[60] md:hidden transition-opacity duration-300" onClick={onClose} />}

      {/* Sidebar Container - Cruzi Neural Gradient (Violet → Purple → Teal) */}
      <div className={`
        w-64 h-screen text-white flex flex-col fixed left-0 top-0 z-[70] 
      ...
      background: 'linear-gradient(180deg, #0f0a1a 0%, #1a0d25 50%, #0d1a1a 100%)'
        ...
        {/* Neural Gradient glow overlay - Violet to Purple to Teal */}
        ...
        {/* Animated glow orbs - Neural Gradient colors */}
        <div className="absolute top-20 -left-10 w-40 h-40 rounded-full blur-3xl animate-pulse pointer-events-none" style={{ backgroundColor: 'rgba(139, 92, 246, 0.25)' }} />
        <div className="absolute bottom-40 -right-10 w-32 h-32 rounded-full blur-3xl animate-pulse pointer-events-none" style={{ backgroundColor: 'rgba(168, 85, 247, 0.2)', animationDelay: '1s' }} />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full blur-3xl pointer-events-none" style={{ backgroundColor: 'rgba(45, 212, 191, 0.15)' }} />

        <div className="relative p-8 flex items-center justify-between">
          <div className="gap-3 flex-row flex items-center justify-center">
            
            <span className="text-2xl font-black tracking-tighter neural-gradient-text">
              Cruzi
            </span>
          </div>
          <button onClick={onClose} className="md:hidden text-white/60 hover:text-white p-2 transition-colors">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <nav className="relative flex-1 px-4 py-4 space-y-1.5 overflow-y-auto custom-scrollbar">
          {menuItems.map((item, index) => {
            ...
            if ('type' in item && item.type === 'section') {
              return (
                <div key={`section-${index}`} className="flex items-center gap-3 px-2 py-3 mt-4 first:mt-0">
            ...
            // Type guard - at this point item must be MenuItem
            ...
            return (
              <button 
                ...
                className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 relative group ${isActive ? 'text-white font-black' : 'text-white/60 hover:text-white'}`} 
                style={isActive ? {
              ...
              >
                {/* Hover glow effect - Neural Gradient */}
                {!isActive && (
                  <div 
                ...
                {menuItem.id === 'messages' && unreadMessageCount > 0 && (
                  <span 
                    className="absolute right-4 top-1/2 -translate-y-1/2 min-w-[20px] h-5 rounded-full text-[10px] font-black flex items-center justify-center px-1.5 text-white" 
                    style={{
        ...
        <div className="relative p-4 mt-auto space-y-4">
          {/* Tier Indicator */}
          <div className="px-2">
            <TierIndicator onUpgradeClick={() => setIsUpgradeModalOpen(true)} />
          </div>

          {/* User Profile Section - Neural Gradient */}
          ...
          boxShadow: '0 0 30px rgba(139, 92, 246, 0.1)'
            ...
            <button onClick={onLogout} className="w-full text-[10px] font-black uppercase tracking-widest py-3 rounded-xl transition-all text-white/60 hover:text-white flex items-center justify-center gap-2" style={{
